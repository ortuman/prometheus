...
func createTestAgentDB(t *testing.T, reg prometheus.Registerer, opts *Options) *DB {
	...
	dbDir := t.TempDir()
	rs := remote.NewStorage(log.NewNopLogger(), reg, startTime, dbDir, time.Second*30, time.Minute*5, nil)
	t.Cleanup(func() {
...
func TestLockfile(t *testing.T) {
	tsdbutil.TestDirLockerUsage(t, func(t *testing.T, data string, createLock bool) (*tsdbutil.DirLocker, testutil.Closer) {
		...
		reg := prometheus.NewRegistry()
		rs := remote.NewStorage(logger, reg, startTime, data, time.Second*30, time.Minute*5, nil)
		t.Cleanup(func() {
			require.NoError(t, rs.Close())
		})
...
func Test_ExistingWAL_NextRef(t *testing.T) {
	dbDir := t.TempDir()
	rs := remote.NewStorage(log.NewNopLogger(), nil, startTime, dbDir, time.Second*30, time.Minute*5, nil)
	defer func() {
		require.NoError(t, rs.Close())
	}()
